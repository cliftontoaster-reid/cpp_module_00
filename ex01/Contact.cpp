...
/*   Updated: 2025/08/06 15:20:25 by lfiorell@st      ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void Contact::setPhoneNumber(const std::string &phoneNumber)
{
  ...
  {
    std::__throw_invalid_argument("Invalid phone number format");
  }
}
...
static void printFieldWithOverflow(const std::string &label, const std::string &value, const std::string &labelColor)
{
  ...
  if (value.length() <= maxFieldWidth)
  {
    ...
    std::cout << RESET << value
              << std::string(maxFieldWidth - value.length(), ' ')
              << BOLD << CYAN << " ║\n"
              << RESET;
  }
  else
    ...
    while (pos < value.length())
    {
      ...
      if (firstLine)
      {
        std::cout << RESET << chunk
                  << std::string(maxFieldWidth - chunk.length(), ' ')
                  << BOLD << CYAN << " ║\n"
                  << RESET;
        firstLine = false;
      }
      else
      {
        ...
        std::cout << BOLD << CYAN << "║ " << RESET
                  ...
                  << std::string(labelWidth - chunk.length(), ' ')
                  << BOLD << CYAN << " ║\n"
                  << RESET;
      }
...
void Contact::displayContact() const
{
  ...
  std::cout << " | "
            << CYAN << formatField(name) << RESET << " | "
            << YELLOW << formatField(firstName) << RESET << " | "
            << MAGENTA << formatField(nickname) << RESET << " |" << std::endl;
}
...
void Contact::displayFullContact() const
{
  ...
  // Title
  std::cout << BOLD << CYAN << "║ " << RESET << BOLD << WHITE << UNDERLINE << "Contact Details" << RESET
            << BOLD << WHITE << std::string(48 - 15, ' ') << BOLD << CYAN << " ║" << "\n"
            << RESET;
